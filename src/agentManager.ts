...
export function getProjectDirPath(cwd?: string): string | null {
	...
	console.log(`[Agents Office] Project dir: ${workspacePath} → ${dirName}`);
	return projectDir;
}
...
export async function launchNewTerminal(
	...
	terminal.sendText(`claude${modelFlag} --session-id ${sessionId}`);

	const projectDir = getProjectDirPath(cwd);
	if (!projectDir) {
		console.log(`[Agents Office] No project dir, cannot track agent`);
		return;
	}
	...
	const pollTimer = setInterval(() => {
		try {
			if (fs.existsSync(agent.jsonlFile)) {
				console.log(`[Agents Office] Agent ${id}: found JSONL file ${path.basename(agent.jsonlFile)}`);
...
export function restoreAgents(
	...
	for (const p of persisted) {
		...
		console.log(`[Agents Office] Restored agent ${p.id} → terminal "${p.terminalName}"`);
		...
		try {
			if (fs.existsSync(p.jsonlFile)) {
				...
				const pollTimer = setInterval(() => {
					try {
						if (fs.existsSync(agent.jsonlFile)) {
							console.log(`[Agents Office] Restored agent ${p.id}: found JSONL file`);
							clearInterval(pollTimer);
