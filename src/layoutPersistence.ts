...
export function readLayoutFromFile(): Record<string, unknown> | null {
	...
	try {
		...
		console.error('[Agents Office] Failed to read layout file:', err);
		return null;
	}
}
...
export function writeLayoutToFile(layout: Record<string, unknown>): void {
	...
	try {
		...
		console.error('[Agents Office] Failed to write layout file:', err);
	}
}
...
export function migrateAndLoadLayout(
	...
	if (fromFile) {
		console.log('[Agents Office] Layout loaded from file');
		return fromFile;
	}
	...
	if (fromState) {
		console.log('[Agents Office] Migrating layout from workspace state to file');
	...
	if (defaultLayout) {
		console.log('[Agents Office] Writing bundled default layout to file');
...
/**
 * Watch ~/.agents-office/layout.json for external changes (other VS Code windows).
...
export function watchLayoutFile(
	...
	function checkForChange(): void {
		...
		try {
			...
			console.log('[Agents Office] External layout change detected');
			onExternalChange(layout);
		} catch (err) {
			console.error('[Agents Office] Error checking layout file:', err);
		}
	}
