...
import {
  ...
  PC_SPRITE,
  LAMP_SPRITE,
  DOOR_CLOSED_FRONT_SPRITE,
  DOOR_OPEN_FRONT_SPRITE,
  DOOR_CLOSED_SIDE_SPRITE,
  DOOR_OPEN_SIDE_SPRITE,
...
// ── Register hardcoded state groups (doors) ─────────────────────
// These are needed when no dynamic assets are loaded (fallback catalog)
function registerHardcodedGroups(): void {
  // State groups: door_front closed ↔ open
  stateGroups.set(FurnitureType.DOOR_FRONT_CLOSED, FurnitureType.DOOR_FRONT_OPEN)
  stateGroups.set(FurnitureType.DOOR_FRONT_OPEN, FurnitureType.DOOR_FRONT_CLOSED)
  offToOn.set(FurnitureType.DOOR_FRONT_CLOSED, FurnitureType.DOOR_FRONT_OPEN)
  onToOff.set(FurnitureType.DOOR_FRONT_OPEN, FurnitureType.DOOR_FRONT_CLOSED)
  // State groups: door_side closed ↔ open
  stateGroups.set(FurnitureType.DOOR_SIDE_CLOSED, FurnitureType.DOOR_SIDE_OPEN)
  stateGroups.set(FurnitureType.DOOR_SIDE_OPEN, FurnitureType.DOOR_SIDE_CLOSED)
  offToOn.set(FurnitureType.DOOR_SIDE_CLOSED, FurnitureType.DOOR_SIDE_OPEN)
  onToOff.set(FurnitureType.DOOR_SIDE_OPEN, FurnitureType.DOOR_SIDE_CLOSED)
}
registerHardcodedGroups()
...
export function buildDynamicCatalog(assets: LoadedAssetData): boolean {
  ...
  // Merge hardcoded door entries (if not already provided by loaded assets)
  const loadedIds = new Set(allEntries.map((e) => e.type))
  const hardcodedDoors = FURNITURE_CATALOG.filter((e) => isDoor(e.type) && !loadedIds.has(e.type))
  if (hardcodedDoors.length > 0) {
    allEntries.push(...hardcodedDoors)
    // Re-register hardcoded door state groups (cleared above)
    registerHardcodedGroups()
    // Track non-front and on-state IDs for the door entries
    for (const entry of hardcodedDoors) {
      if (entry.state === 'on') onStateIds.add(entry.type)
    }
  }
...
/** Door type IDs for quick lookup */
const DOOR_TYPES: Set<string> = new Set([
  FurnitureType.DOOR_FRONT_CLOSED,
  FurnitureType.DOOR_FRONT_OPEN,
  FurnitureType.DOOR_SIDE_CLOSED,
  FurnitureType.DOOR_SIDE_OPEN,
])

/** Returns true if the given furniture type is a door. */
export function isDoor(type: string): boolean {
  return DOOR_TYPES.has(type) || type.startsWith('door_')
}

/** Returns true if the given furniture type is an open door. */
export function isOpenDoor(type: string): boolean {
  return type === FurnitureType.DOOR_FRONT_OPEN || type === FurnitureType.DOOR_SIDE_OPEN || (isDoor(type) && getOffStateType(type) !== type)
}
