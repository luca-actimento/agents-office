...
export function renderBubbles(
  ...
  for (const ch of characters) {
    ...
    if (ch.bubbleType === 'permission') {
      const t = (performance.now() / 1000) % BLINK_ANIMATION_DURATION_SEC
      const phase = t / BLINK_ANIMATION_DURATION_SEC
      alpha = phase < 0.5
        ? 1.0 - (1.0 - 0.15) * (phase / 0.5)
        : 0.15 + (1.0 - 0.15) * ((phase - 0.5) / 0.5)
    } else if (ch.bubbleType === 'waiting' && ch.bubbleTimer < BUBBLE_FADE_DURATION_SEC) {
