...
export function updateCharacter(
  ...
  cafeWalkableTiles?: Array<{ col: number; row: number }>,
): void {
  ch.frameTimer += dt
  ...
  switch (ch.state) {
    case CharacterState.TYPE: {
      ...
      // Café brewing/sitting: stay in TYPE, officeState handles phase transitions
      if (ch.cafePhase === 'brewing' || ch.cafePhase === 'sitting') {
        break
      }
    ...
    case CharacterState.IDLE: {
      ...
      if (ch.wanderTimer <= 0) {
        ...
        // Prefer café zone for idle wandering when available
        const wanderPool = cafeWalkableTiles && cafeWalkableTiles.length > 0
          ? cafeWalkableTiles
          : walkableTiles
    ...
    case CharacterState.WALK: {
      ...
      if (ch.path.length === 0) {
        ...
        // Café walk complete — transition to TYPE, officeState handles phase
        if (ch.cafePhase) {
          ch.state = CharacterState.TYPE
          ch.frame = 0
          ch.frameTimer = 0
        ...
        if (ch.isActive) {
          ...
          if (ch.seatId) {
            ...
            if (seat && ch.tileCol === seat.seatCol && ch.tileRow === seat.seatRow) {
              ch.state = CharacterState.TYPE
              ch.dir = seat.facingDir
              if (ch.skipNextSeatRest) {
                ch.skipNextSeatRest = false
                ch.seatTimer = 0
              } else {
      ...
      // If became active while walking, repath to work seat
