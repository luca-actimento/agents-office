...
// Plugins
import * as dragDropPlugin from '../../plugins/dragDrop.js'
import * as catalogDrag from '../../plugins/furnitureCatalog/catalogDrag.js'
...
interface OfficeCanvasProps {
  ...
  onRotateFurnitureNormal: (uid: string) => void
  editorTick: number
  zoom: number
...
export function OfficeCanvas({ officeState, onClick, isEditMode, editorState, onEditorTileAction, onEditorEraseAction, onEditorSelectionChange, onDeleteSelected, onRotateSelected, onDragMove, onRotateFurnitureNormal, editorTick: _editorTick, zoom, onZoomChange, panRef }: OfficeCanvasProps) {
  ...
  useEffect(() => {
    ...
    const stop = startGameLoop(canvas, {
      ...
      render: (ctx) => {
        ...
        // Plugin ghost previews (drag & drop in normal mode + catalog drag)
        if (!isEditMode) {
          const ddGhost = dragDropPlugin.getGhostState(officeState)
          if (ddGhost) {
            renderGhostPreview(ctx, ddGhost.sprite, ddGhost.col, ddGhost.row, ddGhost.valid, offsetX, offsetY, zoom)
          }
          const catGhost = catalogDrag.getGhostState()
          if (catGhost) {
            renderGhostPreview(ctx, catGhost.sprite, catGhost.col, catGhost.row, catGhost.valid, offsetX, offsetY, zoom)
          }
        }
      },
    })
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      // Plugin: drag-drop move handling (auch außerhalb des Grids für Aufnehmen)
      if (dragDropPlugin.isDragging()) {
        // Unclamped Tile-Position berechnen, damit Drag auch außerhalb funktioniert
        const rawCol = Math.floor(pos.worldX / TILE_SIZE)
        const rawRow = Math.floor(pos.worldY / TILE_SIZE)
        dragDropPlugin.handleMouseMove(rawCol, rawRow)
        const canvas = canvasRef.current
        if (canvas) {
          canvas.style.cursor = 'grabbing'
        }
      ...
      // Plugin: catalog drag ghost update
      if (tile && catalogDrag.isActive()) {
        catalogDrag.updateGhost(officeState, tile.col, tile.row)
        return
      }
      ...
      if (canvas) {
        ...
        if (hitId !== null && !officeState.characters.get(hitId)?.isMascot) {
          ...
          // Plugin: show grab cursor over draggable furniture
          const pluginCursor = dragDropPlugin.getCursor(officeState, tile.col, tile.row)
          if (pluginCursor) {
            cursor = pluginCursor
          } else {
            // Check if hovering over furniture (double-click action)
            const layout = officeState.getLayout()
            const hitFurniture = layout.furniture.find((f) => {
              const entry = getCatalogEntry(f.type)
              if (!entry) return false
            ...
            if (hitFurniture) cursor = 'pointer'
          }
        }
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (!isEditMode) {
        // Plugin: start furniture drag in normal mode
        if (e.button === 0) {
          const tile = screenToTile(e.clientX, e.clientY)
          if (tile && dragDropPlugin.handleMouseDown(officeState, tile.col, tile.row)) {
            return
          }
        }
        return
  ...
  const handleMouseUp = useCallback(
    (e: React.MouseEvent) => {
      ...
      // Plugin: complete furniture drag in normal mode
      if (!isEditMode && dragDropPlugin.isDragging()) {
        dragDropPlugin.handleMouseUp(officeState)
        const canvas = canvasRef.current
  ...
  const handleDoubleClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      for (const f of layout.furniture) {
        ...
        if (!entry) continue
        if (tile.col >= f.col && tile.col < f.col + entry.footprintW && tile.row >= f.row && tile.row < f.row + entry.footprintH) {
          vscode.postMessage({ type: 'furnitureAction', uid: f.uid, furnitureType: f.type })
          return
        }
      }
    },
    [officeState, screenToTile, isEditMode],
  )

  const handleClick = useCallback(
    (e: React.MouseEvent) => {
      if (isEditMode) return // handled by mouseDown/mouseUp
      // Plugin: suppress click if drag was in progress (even without movement)
      if (dragDropPlugin.isDragging()) return
      const pos = screenToWorld(e.clientX, e.clientY)
      if (!pos) return
  ...
  const handleMouseLeave = useCallback(() => {
    ...
    // Plugins
    dragDropPlugin.handleMouseLeave(officeState)
    catalogDrag.leaveCanvas()
  ...
  const handleContextMenu = useCallback((e: React.MouseEvent) => {
    ...
    // Right-click on furniture: rotate it (if no agent selected or not walking)
    if (officeState.selectedAgentId === null) {
      const layout = officeState.getLayout()
      for (const f of layout.furniture) {
        const entry = getCatalogEntry(f.type)
        if (!entry) continue
        if (tile.col >= f.col && tile.col < f.col + entry.footprintW && tile.row >= f.row && tile.row < f.row + entry.footprintH) {
          if (isRotatable(f.type)) {
            onRotateFurnitureNormal(f.uid)
            return
          }
        }
      }
  ...
  }, [isEditMode, officeState, screenToTile, onRotateFurnitureNormal])
  ...
  // HTML5 Drag & Drop handlers for catalog furniture placement
  const handleDragOver = useCallback(
    (e: React.DragEvent) => {
      if (!catalogDrag.isActive()) return
      e.preventDefault()
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent) => {
      ...
      const tile = screenToTile(e.clientX, e.clientY)
      if (tile) {
        catalogDrag.handleDrop(officeState, tile.col, tile.row)
      }
    },
    [officeState, screenToTile],
  )
