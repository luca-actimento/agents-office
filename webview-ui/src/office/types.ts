...
export const FurnitureType = {
  ...
  PC: 'pc',
  LAMP: 'lamp',
  DOOR_FRONT_CLOSED: 'door_front_closed',
  DOOR_FRONT_OPEN: 'door_front_open',
  DOOR_SIDE_CLOSED: 'door_side_closed',
  DOOR_SIDE_OPEN: 'door_side_open',
} as const
...
export interface FurnitureCatalogEntry {
  ...
  /** Rotation/state group identifier */
  groupId?: string
  /** State within group: 'on' | 'off' */
  state?: string
}
...
export interface OfficeLayout {
  ...
  /** Optional zone where idle agents prefer to wander (e.g. a café) */
  cafeZone?: {
    colMin: number; colMax: number; rowMin: number; rowMax: number
    /** Tile where agent stands to use the coffee machine */
    counterTile?: { col: number; row: number }
    /** Direction to face at the counter (Direction enum value) */
    counterFaceDir?: Direction
  }
}
