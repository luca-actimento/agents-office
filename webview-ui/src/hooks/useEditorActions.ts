...
export interface EditorActions {
  ...
  handleRotateFurnitureNormal: (uid: string) => void
}
...
export function useEditorActions(
  ...
  /** Rotate furniture in normal mode (right-click) — no undo stack */
  const handleRotateFurnitureNormal = useCallback((uid: string) => {
    const os = getOfficeState()
    const layout = os.getLayout()
    const newLayout = rotateFurniture(layout, uid, 'cw')
    if (newLayout !== layout) {
      os.rebuildFromLayout(newLayout)
      saveLayout(newLayout)
    }
